...
const CompaniesParamsSchema = z.object({
...
export class ToolHandlers {
    ...
    async listCompanies(args) {
        ...
        const companiesList = companies.map((c, idx) => `${idx + 1}. ID: ${c.id} - "${c.title || c.public_title}"\n   Address: ${c.address || 'N/A'}\n   Phone: ${c.phone || 'N/A'}`).join('\n\n');
        return {
            content: [{
                    type: 'text',
    ...
    async getBookings(args) {
        ...
        const bookingsList = bookings.map((b, idx) => `${idx + 1}. Booking ID: ${b.id}\n` +
    ...
    async getServices(args) {
        ...
        const servicesList = services.map((s, idx) => `${idx + 1}. ID: ${s.id} - "${s.title}"\n` +
            `   Price: ${s.cost}${s.duration ? `\n   Duration: ${s.duration} min` : ''}${s.category_id ? `\n   Category ID: ${s.category_id}` : ''}`).join('\n\n');
        return {
            content: [{
                    type: 'text',
    ...
    async getServiceCategories(args) {
        ...
        const categoriesList = categories.map((c, idx) => `${idx + 1}. ID: ${c.id} - "${c.title}"${c.services ? `\n   Services count: ${c.services.length}` : ''}`).join('\n\n');
        return {
            content: [{
                    type: 'text',
    ...
    async getSchedule(args) {
        ...
        const scheduleList = schedule.map((s, idx) => `${idx + 1}. ${s.date} at ${s.time} (${s.seance_length} min)`).join('\n');
        return {
            content: [{
                    type: 'text',
