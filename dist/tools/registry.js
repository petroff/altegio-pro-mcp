...
const tools = [
    ...
    },
    {
        ...
        inputSchema: {
            type: 'object',
            properties: {
                company_id: { type: 'number', description: 'ID of the company to get bookings for' },
                page: { type: 'number', description: 'Page number for pagination (starts at 0). Use to fetch subsequent pages when user needs more results.' },
                count: { type: 'number', description: 'Results per page. Default may be large. RECOMMENDED: Use 20-50 for initial requests. Only increase if user explicitly requests more. Max 300.' },
                start_date: { type: 'string', description: 'Filter bookings from this date (YYYY-MM-DD format). Use to reduce result set.' },
                end_date: { type: 'string', description: 'Filter bookings until this date (YYYY-MM-DD format). Use to reduce result set.' }
            },
            required: ['company_id']
        }
    },
    {
        ...
        inputSchema: {
            type: 'object',
            properties: {
                company_id: { type: 'number', description: 'ID of the company to get staff list for' },
                page: { type: 'number', description: 'Page number for pagination (starts at 0). Use to fetch subsequent pages when user needs more results.' },
                count: { type: 'number', description: 'Results per page. Default may be large. RECOMMENDED: Use 30-50 for initial display. Max 300.' }
            },
            required: ['company_id']
        }
    },
    {
        ...
        inputSchema: {
            type: 'object',
            properties: {
                company_id: { type: 'number', description: 'ID of the company to get services for' },
                page: { type: 'number', description: 'Page number for pagination (starts at 0). Use to fetch subsequent pages when user needs more results.' },
                count: { type: 'number', description: 'Results per page. Default may be large. RECOMMENDED: Use 30-50 for initial display. Max 300.' }
            },
            required: ['company_id']
    ...
    },
    {
        ...
        inputSchema: {
            type: 'object',
            properties: {
                company_id: { type: 'number', description: 'ID of the company to get service categories for' },
                page: { type: 'number', description: 'Page number for pagination (starts at 0). Use to fetch subsequent pages when user needs more results.' },
                count: { type: 'number', description: 'Results per page. Default may be large. RECOMMENDED: Use 20-30 for initial display. Max 300.' }
            },
            required: ['company_id']
        }
    },
